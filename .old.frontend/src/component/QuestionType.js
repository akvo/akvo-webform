...
class QuestionType extends Component {
  ...
  handleFile(event) {
    ...
    reader.addEventListener("load", () => {
      db.files
        .put({ id: fileId, fileName: file.name, blob: reader.result, qid: id })
        .then(() => {
  ...
  handleChange(event) {
    ...
    if (this.props.data.type === "cascade") {
      ...
      this.props.data.options.allowMultiple
    ) {
      let multipleValue = [];
      let existValue = [];
      ...
      let childs = this.props.value.questions.filter(
        (x) => x.dependency !== undefined
      );
  ...
  getRadio(data, unique) {
    ...
    let arrayOpt = Array.isArray(data.options.option)
      ? data.options.option
      : [data.options.option];
    let opts = {
      ...data.options,
  ...
  renderRadio(o, lang, opt, i, id, radioType, unique) {
    ...
    localization =
      localization.length === 0 ? lang.en : localization.join(" / ");
    let dataval =
      opt.code !== undefined
    ...
    return (
      <div className="form-check" key={unique + "-div-radio-" + i.toString()}>
        <input
          key={unique + "-radio-" + i.toString()}
          className="form-check-input"
          type={radioType}
  ...
  renderCascade(opt, i, l, unique, id) {
      ...
      dropdown !== undefined ? (
        <div key={unique + "-dropdown-" + i}>
          ...
          <select
            ...
            onChange={this.handleChange}
          >
            <option key={unique + "-cascade-options-" + 0} value="">
              Please Select
            </option>
    ...
    if (
      dropdown === undefined &&
      this.state.cascade_selected[i - 1] !== undefined
    ) {
      if (
        this.state[
          "options_" + this.state.cascade_selected[i - 1].toString()
        ] === undefined
      ) {
        setTimeout(() => {
          this.getCascadeDropdown(
            this.state.cascade_selected[i - 1],
            i,
            0,
            true
          );
  ...
  getCascadeDropdown(lv, ix, selected_value, init) {
    if (this.props.data.type === "cascade") {
      let url =
        API_URL +
        "cascade/" +
        this.props.value.instanceName +
        "/" +
        this.props.data.cascadeResource +
        "/" +
        lv;
      ...
      if (isavailable) {
        res = availcasc.find((x) => {
          return x.question === this.props.data.id && x.url === url;
        });
      }
      if (!res) {
  ...
  getInput(data, unique, validation, answered, type) {
    ...
    if (this.state.invalid) {
      validator = validation.minVal
        ? "Min Value (" + parseInt(validation.minVal) + ") "
        : validator;
      validator = validation.maxVal
        ? validator + "Max Value (" + parseInt(validation.maxVal) + ")"
        : validator;
    }
    let unit = data.help
      ? data.help.text !== null
        ? data.help.text.includes("##UNIT##")
        : false
      : false;
    if (unit) {
      unit = unit
  ...
  getGeo(data, unique) {
    ...
    return <MapForm center={pos} zoom={13} update={this.handleMapChange} />;
  }
  ...
  componentDidMount() {
    if (this.props.data.type === "cascade") {
      ...
      selected_cascade =
        selected_cascade !== null ? JSON.parse(selected_cascade) : false;
      if (selected_cascade) {
