...
class Home extends Component {
  ...
  constructor(props) {
    ...
    this.cacheId =
      this.props.match.params.cacheid === undefined
        ? false
        : this.props.match.params.cacheid;
  ...
  redirectError() {
    PopupError("URL is not available, redirecting page").then((res) => {
  ...
  getCachedSurvey() {
    axios
      .get(API_URL + "form-instance/" + this.cacheId)
      .then((res) => {
        ...
        let formChanged =
          localStorage.getItem("_cache") === this.cacheId ? true : false;
        let changes = [];
        if (formChanged) {
          ...
          questions.forEach((id) => {
            if (stored[id] !== undefined && answers[id] !== undefined) {
              changes = [...changes, stored[id] !== answers[id]];
            }
          });
        }
  ...
  render() {
    return (
      <div
        className={
          this.state._fullscreen ? "wrapper d-flex toggled" : "wrapper d-flex"
        }
      >
