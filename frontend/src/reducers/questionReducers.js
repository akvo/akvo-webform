...
const DOMAIN_ID =
  localStorage.getItem("_cache") !== null
    ? localStorage.getItem("_cache")
    : false;
const DOMAIN = DOMAIN_ID
  ? clearDomain(DOMAIN_ID) + "/" + DOMAIN_ID
  : document.referrer;
...
const getGroupAttributes = (group, questions, answers) => {
  ...
  questions = questions.filter((x) => {
    ...
    if (x.dependency) {
      ...
      answer_dependency = answer_dependency
        ? answer_dependency.map((x) => x.text)
        : answer_dependency;
  ...
  mandatories =
    mandatories.length === 0
      ? 0
      : questions.filter((x) => x.mandatory).length - mandatories.length;
...
const validateGroup = (data, sumber) => {
  ...
  if (typeof data === "object") {
    let question = Array.isArray(data.question)
      ? data.question
      : [data.question];
    groups.push({
      index: 0,
...
const listDatapoints = (data) => {
  ...
  return addQuestions(groups)
    .filter((x) => x.localeNameFlag)
    .map((q) => q.id);
};
...
const cloneQuestions = (questions, groups, group_id, restoring = false) => {
  ...
  do {
    ...
    if (new_questions[i].dependency !== undefined) {
      ...
      let dependency_iteration =
        group.questions.indexOf(dependency_question) === -1
          ? "0"
          : new_iteration;
      new_questions[i] = {
        ...new_questions[i],
        dependency: {
          ...
          question:
            new_questions[i].dependency.question.split("-")[0] +
            "-" +
...
const removeQuestions = (questions, groups, data) => {
  ...
  let new_iterated = selected_questions.reduce(
    (h, obj) =>
      Object.assign(h, {
        [obj.iteration]: (h[obj.iteration] || []).concat(obj),
      }),
    {}
  );
  let new_questions = [];
  Object.keys(new_iterated).forEach(function (k, ix) {
    new_iterated[k].forEach((q) => {
      ...
      if (q.dependency !== undefined) {
        ...
        let dependency_iteration =
          qgroup.questions.indexOf(default_dependency) === -1 ? "0" : ix;
        q = {
          ...q,
          dependency: {
            ...q.dependency,
            question:
              q.dependency.question.split("-")[0] + "-" + dependency_iteration,
          },
        };
      }
...
const updateLocalStorage = (questions) => {
  ...
  localStorage.setItem(
    "answerType",
    questions.map((x) => x.type.toUpperCase())
  );
  return true;
};
...
export const questionReducers = (state = initialState, action) => {
  switch (action.type) {
    ...
    case "REMOVE GROUP":
      const uncloned = removeQuestions(
        state.questions,
        state.groups,
        action.data
      );
      const removedQuestions = state.questions.filter(
        (x) => !uncloned.questions.map((u) => u.id).includes(x.id)
      );
      const currentCascade = state.cascade.filter(
        (x) => !removedQuestions.map((r) => r.id).includes(x.question)
      );
      return {
        ...state,
